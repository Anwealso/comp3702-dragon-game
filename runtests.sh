...
#    python3 simulator.py testcases/a3-t$1.txt 2> temp.txt;
    python3 simulator.py testcases/a3-t$1.txt;
    ...
    do
        echo "[[ Test $i: ]]"
        # Run the simulator script
#        python3 simulator.py testcases/a3-t$i.txt 2> temp.txt;
        python3 simulator.py testcases/a3-t$i.txt;
        # Save the output status (255=crash, 254=overtime, otherwise returns the score /50)
        echo $? > temp.txt
        exitcode=$(cat temp.txt)
        echo "[[ Exit status: $exitcode ]]"
